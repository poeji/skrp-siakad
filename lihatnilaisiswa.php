        ...
        <table width="708" border="0" cellpadding="0" cellspacing="0" class="tinytable" id="table">
            <thead>
							  <tr>
                                  ...
                                  <th width="60%"><h3>Nama Siswa</h3></th>
...
$plustahun = (date("Y"))."-".(date("Y")+1);
						  		...
						  		$q = mysql_query("SELECT s.`nis`, s.`nama_siswa`, k.`nama_kelas`, k.kode_kelas, s.`tahun`
FROM `siswa` s
LEFT JOIN `kelas` k ON k.`kode_kelas` = s.`kode_kelas`
RIGHT JOIN `nilai_pengetahuan` p ON p.`kode_kelas` = s.`kode_kelas` AND p.`tahun_ajaran` = s.`tahun`
WHERE p.`nip` = '".$_SESSION["userid"]."'
GROUP BY s.`nis`
ORDER BY s.`nama_siswa` ASC");
						  ...
						  ?>	
							<tr>	
								...
								<td class="center">
									<a class="btn btn-info" href="?page=listdatanilaisiswadetail&kelas=<?php echo $d['kode_kelas']; ?>&nis=<?php echo $d['nis']; ?>&tahun=<?php echo $d['tahun']; ?>" title="Lihat Detail Nilai Pengetahuan">
								 		Detail  
									</a>
								</td>
							</tr>
